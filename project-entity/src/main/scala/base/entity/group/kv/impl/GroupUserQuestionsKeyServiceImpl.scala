...
class GroupUserQuestionsKeyServiceImpl
    extends SetKeyServiceImpl[OrderedIdPair, GroupUserQuestionsKey]
    with GroupUserQuestionsKeyService {

  def make(a: UUID, b: UUID)(implicit p: Pipeline) = make(OrderedIdPair(a, b))

  def make(id: OrderedIdPair)(implicit p: Pipeline) = new GroupUserQuestionsKeyImpl(getKey(id), this)
